...
public class CinemachineCollisionHandler : MonoBehaviour
{
    ...
    public void Initialize(CinemachineFreeLook cam)
    {
        camera = cam;
    ...
    public void UpdateCameraClipPoints(Vector3 cameraPosition, Quaternion atRotation, ref Vector3[] newClipPointsArray)
    {
        ...
        float z = camera.m_Lens.NearClipPlane; // distance from the camera position to the new clip plane.
        float x = Mathf.Tan(camera.m_Lens.FieldOfView / 3.41f) * z; // changing the 3.41 will change the size of the collision space.
        float y = x / camera.m_Lens.Aspect;
